...
The `home-routes` handler in the `eedcpower.routes.home` namespace
...
The `home-page` function will in turn call the `eedcpower.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `eedcpower.handler` namespace:
...
Request middleware functions are located under the `eedcpower.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `eedcpower.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `eedcpower.db.core` in some other namespace.
